<?php

return [

    'brand' => 'Go CMS',

    'powered' => [
        'lead' => 'Powered by',
        'tags' => 'Strumenti per il tuo business · AI · CRM su misura',
    ],

    'nav' => [
        'groups' => [
            'fundraising' => 'Raccolte fondi',
            'content' => 'Contenuti',
            'settings' => 'Impostazioni',
        ],
        'pages' => 'Pagine',
        'news' => 'Notizie',
        'news_categories' => 'Categorie notizie',
        'editorial' => 'Articoli',
        'editorial_categories' => 'Categorie articoli',
        'users' => 'Utenti',
        'campaigns' => 'Campagne donazioni',
        'integrations' => 'Integrazioni',
        'sportelli_config' => 'Sportelli',
        'captcha' => 'Captcha',
        'social' => 'Social',
        'appearance' => 'Aspetto',
        'donations_config' => 'Donazioni (5 x 1000 / IBAN)',
        'recurring_donations' => 'Donazioni ricorrenti',
        'site_content' => 'Contenuto del sito',
        'developer_tools' => 'Strumenti sviluppatore',
    ],

    'models' => [
        'page' => 'pagina',
        'pages' => 'pagine',
        'article' => 'articolo',
        'articles' => 'articoli',
        'news_article' => 'notizia',
        'news_articles' => 'notizie',
        'editorial_article' => 'articolo',
        'editorial_articles' => 'articoli',
        'editorial_category' => 'categoria articoli',
        'editorial_categories' => 'categorie articoli',
        'user' => 'utente',
        'users' => 'utenti',
        'article_category' => 'categoria notizie',
        'article_categories' => 'categorie notizie',
        'donation_campaign' => 'campagna donazioni',
        'donation_campaigns' => 'campagne donazioni',
    ],

    'fields' => [
        'title' => 'Titolo',
        'slug' => 'Slug',
        'body' => 'Testo',
        'name' => 'Nome',
        'first_name' => 'Nome',
        'last_name' => 'Cognome',
        'full_name' => 'Nome completo',
        'job_title' => 'Qualifica / ruolo',
        'description' => 'Descrizione',
        'excerpt' => 'Estratto',
        'category' => 'Categoria',
        'author' => 'Autore',
        'email' => 'Email',
        'password' => 'Password',
        'role' => 'Ruolo',
        'published' => 'Pubblicato',
        'published_at' => 'Data di pubblicazione',
        'show_author' => 'Mostra nome e qualifica dell\'autore',
        'key' => 'Chiave stabile',
        'template' => 'Modello di pagina',
        'updated' => 'Aggiornato',
        'image' => 'Immagine',
        'slides' => 'Slide',
        'batch_upload' => 'Carica più foto',
        'alt_text' => 'Testo alternativo (:locale)',
        'language' => 'Lingua',
        'value' => 'Valore',
        'label_locale' => 'Etichetta (:locale)',
        'title_locale' => 'Titolo (:locale)',
        'body_locale' => 'Testo (:locale)',
        'stats_locale' => 'Riga statistiche (:locale)',
        'cards' => 'Schede',
        'stats' => 'Statistiche',
        'currency' => 'Valuta',
        'sort_order' => 'Ordine',
        'is_active' => 'Attiva',
        'min_amount_cents' => 'Importo minimo (centesimi)',
        'allow_custom_amount' => 'Consenti importo personalizzato',
        'recurring_donation_enabled' => 'Mostra donazione ricorrente sul sito',
        'preset_amounts' => 'Importi predefiniti (EUR)',
        'form_notice' => 'Avviso modulo (dati carta)',
        'privacy_notice' => 'Contenuto pagina privacy',
        'thank_you_message' => 'Messaggio di ringraziamento',
        'fundraising_goal' => 'Importo obiettivo (EUR)',
        'espocrm_finanziamento_name' => 'Nome Finanziamento EspoCRM',
        'section_label' => 'Etichetta sezione (rosso)',
        'hero_eyebrow' => 'Occhiello hero',
        'hero_tagline' => 'Slogan hero',
        'our_values' => 'I nostri valori',
        'closing_statement' => 'Frase conclusiva',
        'donate_button' => 'Etichetta pulsante dona',
        'volunteer_button' => 'Etichetta pulsante volontariato',
        'social_instagram' => 'Instagram',
        'social_facebook' => 'Facebook',
        'social_x' => 'X',
        'social_whatsapp' => 'WhatsApp',
        'social_email' => 'Email',
        'social_tiktok' => 'TikTok',
        'site_background' => 'Sfondo personalizzato',
        'background_current' => 'Sfondo attuale',
        'background_stock_option' => 'Foto stock (default)',
        'background_custom_option' => 'Sfondo personalizzato',
        'custom_background' => 'Sfondo personalizzato',
        'background_from_library' => 'Scegli tra i file già caricati',
        'background_upload' => 'Oppure carica un nuovo file',
        'primary_tagline' => 'Slogan principale',
        'home_independence_title' => 'Titolo banner indipendenza',
        'home_independence_body' => 'Testo banner indipendenza',
        'publishable_key' => 'Chiave pubblicabile',
        'secret_key' => 'Chiave segreta',
        'webhook_secret' => 'Segreto firma webhook',
        'default_currency' => 'Valuta predefinita',
        'statement_descriptor' => 'Suffisso estratto conto',
        'account_id' => 'ID account (riferimento)',
        'account_name' => 'Nome account (riferimento)',
        'customer_portal_login_url' => 'URL login Customer Portal Stripe',
        'base_url' => 'URL base',
        'api_key' => 'Chiave API',
        'assigned_user_id' => 'Utente assegnato (CRM)',
        'assigned_user_none' => 'Nessuno (default CRM)',
        'assigned_user_site_api' => 'Sito API — :name',
        'assigned_user_unknown' => 'Utente salvato (:id) — non in elenco',
        'assigned_user_manual_id' => 'ID utente CRM',
        'beneficiary_name' => 'Nome beneficiario',
        'default_payer_name' => 'Nome pagatore predefinito',
        'smtp_host' => 'Host SMTP',
        'smtp_port' => 'Porta SMTP',
        'smtp_encryption' => 'Crittografia',
        'smtp_encryption_none' => 'Nessuna',
        'smtp_username' => 'Utente SMTP',
        'smtp_password' => 'Password SMTP',
        'smtp_provider_preset' => 'Provider email',
        'smtp_provider_custom' => 'Personalizzato (inserimento manuale)',
        'from_address' => 'Email mittente',
        'from_name' => 'Nome mittente',
        'contact_website_from_address' => 'Mittente modulo contatti (sito)',
        'contact_website_from_name' => 'Nome mittente modulo contatti',
        'contact_desks' => 'Sportelli',
        'contact_desk_key' => 'Chiave interna',
        'contact_desk_label' => 'Etichetta nel menu',
        'contact_desk_inbox' => 'Email di destinazione (group inbox CRM)',
        'contact_desk_case_type' => 'Tipo segnalazione CRM (Case.type)',
        'sportelli_config_link' => 'Configurazione sportelli',
        'sportello_mail_subject' => 'Oggetto email',
        'sportello_mail_body' => 'Corpo email (HTML)',
        'sportello_mail_placeholders' => 'Segnaposto disponibili',
        'turnstile_enabled' => 'Abilita captcha Cloudflare Turnstile (opzionale)',
        'turnstile_site_key' => 'Turnstile site key',
        'turnstile_secret_key' => 'Turnstile secret key',
        'five_per_mille_enabled' => 'Mostra pagina 5 x 1000',
        'codice_fiscale' => 'Codice fiscale associazione',
        'five_per_mille_menu_label' => 'Etichetta menu',
        'five_per_mille_heading' => 'Titolo pagina',
        'five_per_mille_lead' => 'Sottotitolo',
        'five_per_mille_body' => 'Testo introduttivo (HTML)',
        'five_per_mille_instructions' => 'Istruzioni per la dichiarazione (HTML)',
        'five_per_mille_codice_label' => 'Etichetta codice fiscale',
        'bank_transfer_enabled' => 'Mostra sezione bonifico',
        'donation_iban' => 'IBAN',
        'donation_beneficiary' => 'Intestatario conto',
        'bank_transfer_heading' => 'Titolo sezione bonifico',
        'bank_transfer_body' => 'Testo bonifico (HTML)',
        'bank_transfer_iban_label' => 'Etichetta IBAN',
        'bank_transfer_beneficiary_label' => 'Etichetta intestatario',
        'no_cache' => 'Disattiva cache browser (No cache)',
    ],

    'integrations' => [
        'stripe' => 'Stripe',
        'espocrm' => 'EspoCRM',
        'mail' => 'Email',
    ],

    'sections' => [
        'translations' => 'Traduzioni',
        'photo_carousel' => 'Carosello foto',
        'hero_carousel' => 'Carosello hero',
        'service_cards' => 'Schede servizi',
        'social_links' => 'Link social',
        'site_background' => 'Sfondo del sito',
        'page_background' => 'Sfondo pagina',
        'site_tagline' => 'Slogan del sito',
        'home_independence' => 'Banner indipendenza (home)',
        'stripe_payments' => 'Pagamenti Stripe',
        'crm_api' => 'API CRM',
        'prima_nota_defaults' => 'Predefiniti Prima Nota',
        'smtp' => 'Server SMTP',
        'mail_sender' => 'Mittente',
        'contact_notifications' => 'Modulo contatti (Sportelli)',
        'contact_desks' => 'Elenco sportelli',
        'sportello_mail' => 'Email automatiche',
        'contact_captcha' => 'Captcha modulo contatti',
        'five_per_mille' => '5 x 1000',
        'bank_transfer' => 'Bonifico bancario',
        'recurring_donation' => 'Donazione ricorrente',
        'developer_cache' => 'Cache e sviluppo',
    ],

    'helpers' => [
        'stable_key' => 'Usata per i link di navigazione diretti (chi siamo, servizi, contatti). Le pagine senza queste chiavi compaiono automaticamente in «Altre pagine» nel menu quando sono pubblicate.',
        'page_template' => 'Ogni modello ha un layout pubblico distinto — salva la pagina, poi usa Anteprima (IT/EN).',
        'published_at' => 'Obbligatoria affinché l\'articolo compaia su /notizie.',
        'show_author' => 'Se disattivato, sul sito non compare «Pubblicato da …» anche se l\'articolo è pubblicato.',
        'article_carousel' => 'Galleria facoltativa mostrata nella pagina articolo e nel feed notizie.',
        'batch_upload' => 'Seleziona più immagini: al termine del caricamento vengono aggiunte automaticamente alla galleria.',
        'slide_image' => 'JPEG, PNG o WebP. Fino a 25 MB in upload; salvataggio automatico entro 8 MB.',
        'carousel_repeater' => 'Le slide senza immagine vengono ignorate al salvataggio. Puoi aggiungere il testo alternativo dopo.',
        'hero_carousel' => 'Galleria fotografica facoltativa in cima a qualsiasi modello di pagina. Visibile solo se è caricata almeno un\'immagine.',
        'section_label' => 'Piccola etichetta o banner rosso per questa pagina. Lascia vuoto per il predefinito.',
        'hero_eyebrow' => 'Piccola riga rossa sopra il titolo principale.',
        'hero_tagline' => 'Breve riga accanto al titolo grande.',
        'our_values' => 'Colonna destra — pannello valori evidenziato.',
        'closing_statement' => 'Citazione a tutta larghezza in fondo alla pagina.',
        'social_links' => 'Compila solo i canali attivi. Compariranno come icone nel footer e sotto il banner home.',
        'social_instagram' => 'URL completo del profilo Instagram.',
        'social_facebook' => 'URL completo della pagina Facebook.',
        'social_x' => 'URL completo del profilo X (Twitter).',
        'social_whatsapp' => 'Link wa.me o URL WhatsApp Business.',
        'social_email' => 'Indirizzo email pubblico (es. info@safehouse.community).',
        'social_tiktok' => 'URL completo del profilo TikTok.',
        'site_background' => 'Di default è selezionata la foto stock. Puoi sostituirla con JPG/PNG/WebP/GIF/SVG oppure ripristinare i poligoni Aurora.',
        'site_background_upload' => 'JPG, PNG, WebP, GIF (anche animati) o SVG — max 8 MB. Consigliato formato orizzontale. Rimuovi il file per tornare allo stock (o usa Ripristina Aurora).',
        'restore_aurora_background' => 'Rimuove lo sfondo attuale e applica i poligoni Safehouse Aurora.',
        'background_using_aurora' => 'In uso: poligoni Safehouse Aurora (nessuna foto).',
        'page_background' => 'Opzionale: sostituisce lo sfondo globale solo su questa pagina.',
        'custom_background' => 'Attiva per scegliere o caricare uno sfondo diverso dal globale.',
        'background_from_library' => 'File già presenti in Impostazioni → Aspetto o caricati qui in precedenza.',
        'background_upload' => 'Se carichi un file, ha priorità sulla selezione dalla libreria.',
        'service_cards' => 'Schede numerate nella griglia sotto il banner rosso introduttivo.',
        'stats_line' => 'Riga facoltativa in fondo, es. «1.000+ pasti caldi · volontari».',
        'stat_value' => 'Numero o segnaposto, es. 1.000+ o —',
        'about_body' => 'Colonna sinistra — testo principale della missione.',
        'services_body' => 'Testo nel banner rosso sopra le schede.',
        'preset_amounts' => 'Importi in euro. 5 = €5,00, 12,50 = €12,50. Memorizzati internamente in centesimi.',
        'thank_you_message' => 'Mostrato dopo un pagamento riuscito. Lascia vuoto per il testo predefinito del sito.',
        'fundraising_goal' => 'Obiettivo della raccolta in CRM (Fondi e Finanziamenti → Importo) quando il record non esiste ancora. Lascia vuoto per 0.',
        'recurring_donation_enabled' => 'Se attivo, la scheda e la pagina /donazioni/donazione-ricorrente sono pubbliche. Nessun obiettivo di raccolta: solo titolo e descrizione.',
        'espocrm_finanziamento' => 'Compilato automaticamente al salvataggio della campagna. Modifica solo se il nome in CRM deve differire dal titolo della campagna.',
        'primary_tagline' => 'Slogan nel footer (es. Comunità di accoglienza…). Il motto Accogliere / Includere resta solo nel manifesto al centro della home.',
        'site_tagline' => 'Slogan del footer. Non usare qui il motto Accogliere — quello sta solo al centro della home.',
        'home_independence' => 'Mostrato sulla home tra la citazione e i contatori di impatto.',
        'home_independence_title' => 'Parola in evidenza (es. Indipendenza).',
        'home_independence_body' => 'Testo dopo i due punti.',
        'stripe_publishable' => 'pk_test_… per i test, pk_live_… per la produzione.',
        'stripe_secret' => 'Memorizzata cifrata. Lascia vuoto per mantenere il valore attuale al salvataggio.',
        'stripe_webhook' => 'whsec_… dalla Dashboard Stripe o stripe listen.',
        'stripe_webhook_live' => 'whsec_… dall\'endpoint LIVE in Stripe Dashboard (Developers → Webhooks). Il segreto di test non funziona con chiavi live.',
        'stripe_descriptor' => 'Aggiunto sugli estratti conto dei donatori (max 22 caratteri). Stripe usa statement_descriptor_suffix sui PaymentIntent.',
        'stripe_account_id' => 'Facoltativo. Mostrato nell\'URL della Dashboard; verificato con php artisan stripe:verify.',
        'stripe_account_name' => 'Etichetta facoltativa per il team — non inviata all\'API Stripe.',
        'customer_portal_login_url' => 'Dashboard Stripe → Settings → Billing → Customer portal → copia il link della pagina di login (billing.stripe.com/p/login/…). Test e live hanno URL diversi.',
        'espocrm_assigned_user' => 'A chi assegnare Prima Nota / Lead creati dal sito. Vuoto = default CRM. «Sito API» = utente della chiave API. Altri utenti solo se l\'API può leggerli (altrimenti il sito ripiega sul Sito API). Puoi anche inserire un ID manualmente.',
        'beneficiary_name' => 'Ricerca nome conto in CRM, es. Safe House.',
        'default_payer_name' => 'Fallback quando il nome del donatore è vuoto.',
        'integrations_subheading' => 'I valori salvati qui sostituiscono .env sul server. I segreti sono cifrati nel database.',
        'smtp_host' => 'Es. smtp.gmail.com, mail.safehouse.community o il server del tuo provider.',
        'smtp_provider_preset' => 'Scegli un provider e clicca «Applica preset» — restano solo utente e password.',
        'smtp_port' => 'Di solito 587 con TLS oppure 465 con SSL.',
        'smtp_password' => 'Memorizzata cifrata. Lascia vuoto per mantenere il valore attuale al salvataggio.',
        'from_address' => 'Indirizzo mostrato come mittente delle email automatiche del sito.',
        'contact_notification_email' => 'Obsoleto — il modulo contatti invia solo verso Sportello digitale o legale.',
        'contact_website_from_address' => 'Indirizzo mittente per le richieste dal sito (es. website@safehouse.community). Usa le credenziali SMTP della sezione sopra.',
        'contact_website_from_name' => 'Nome mostrato come mittente nelle email verso gli sportelli.',
        'contact_desks' => 'Ogni sportello invia email al group inbox indicato. In CRM configura manualmente lo stesso indirizzo, il team e il tipo segnalazione corrispondente.',
        'contact_desk_key' => 'Identificatore stabile (es. digital_desk). Non cambiarlo dopo il go-live se ci sono già richieste.',
        'contact_desk_case_type' => 'Valore enum Case.type in EspoCRM (es. SportelloDigitale). Deve esistere nei metadati CRM. Integrazione CRM non disponibile: elenco di fallback.',
        'contact_desk_case_type_crm' => 'Elenco caricato da EspoCRM (Case.type). Scegli il tipo segnalazione corrispondente allo sportello.',
        'sportelli_config_link' => 'Sportelli e template email si configurano in Impostazioni → Sportelli. Il captcha è in Impostazioni → Captcha.',
        'captcha_now_in_settings' => 'Il captcha non si configura più qui. Vai su Impostazioni → Captcha.',
        'captcha_subheading' => 'Chiavi Cloudflare Turnstile per i moduli pubblici (contatti e volontari). Il segreto è cifrato. Lascia vuoto il campo segreto per mantenere il valore attuale.',
        'sportello_mail_subject' => 'Facoltativo. Usa i segnaposto elencati sotto. Se vuoto: [SH-token] Nuovo messaggio — nome.',
        'sportello_mail_body' => 'Editor HTML come per le pagine del sito. Alla fine viene sempre aggiunto un blocco testo per CRM (Sportello, Tipo segnalazione, Riferimento SH).',
        'turnstile_enabled' => 'Disattivato per impostazione predefinita. Attivalo solo se hai configurato le chiavi Turnstile su Cloudflare.',
        'turnstile_site_key' => 'Chiave pubblica (widget sul sito).',
        'turnstile_secret_key' => 'Chiave segreta (verifica server). Memorizzata cifrata.',
        'five_per_mille_enabled' => 'Pagina /donazioni/5-per-mille e voce in evidenza nel menu.',
        'codice_fiscale' => 'Codice da indicare nel riquadro 5 x 1000 della dichiarazione dei redditi.',
        'five_per_mille_menu_label' => 'Testo nel menu principale (in evidenza).',
        'bank_transfer_enabled' => 'Blocco bonifico in cima alla pagina /donazioni.',
        'donation_iban' => 'IBAN mostrato sul sito per i bonifici diretti.',
        'developer_cache' => 'Come in EspoCRM: disattiva la cache HTTP del browser e svuota le cache Laravel. Non ricostruisce gli asset Vite (usa bin/dev-rebuild-frontend.sh).',
        'no_cache' => 'Aggiunge Cache-Control: no-store alle risposte HTTP. Utile in sviluppo quando HTML/CSS restano vecchi dopo un rebuild.',
    ],

    'templates' => [
        'default' => [
            'label' => 'Pagina semplice',
            'description' => 'Titolo, area introduttiva e corpo ricco in un unico pannello.',
            'layout' => 'Layout: titolo + pannello singolo.',
        ],
        'about' => [
            'label' => 'Chi siamo',
            'description' => 'Intro missione, blocco valori evidenziato, frase conclusiva.',
            'layout' => 'Layout: 2 colonne + pannello valori + citazione.',
        ],
        'services' => [
            'label' => 'Griglia servizi',
            'description' => 'Intro più griglia di schede da meta strutturato (services[].title/body).',
            'layout' => 'Layout: banner rosso + schede numerate.',
        ],
        'article' => [
            'label' => 'Articolo / lettura lunga',
            'description' => 'Layout editoriale per saggi e contenuti approfonditi.',
            'layout' => 'Layout: colonna stretta + capolettera.',
        ],
        'landing' => [
            'label' => 'Landing',
            'description' => 'Hero ampio ed enfasi facoltativa sulle call-to-action.',
            'layout' => 'Layout: hero completo + pulsanti CTA.',
        ],
        'legal' => [
            'label' => 'Legale',
            'description' => 'Privacy, cookie e pagine informative.',
            'layout' => 'Layout: documento legale monospace.',
        ],
        'contact' => [
            'label' => 'Contatti',
            'description' => 'Dettagli di contatto e segnaposto modulo.',
            'layout' => 'Layout: informazioni + modulo.',
        ],
        'home' => [
            'label' => 'Home page',
            'description' => 'Hero pubblico, CTA e statistiche d\'impatto.',
            'layout' => 'Layout: hero, CTA e contatori.',
        ],
        'example' => ' Esempio live: chiave pagina «:key».',
        'fallback' => 'Ogni modello ha un layout pubblico distinto — salva la pagina, poi usa Anteprima (IT/EN).',
    ],

    'body_labels' => [
        'default' => 'Testo',
        'about' => 'Intro missione',
        'services' => 'Intro servizi',
    ],

    'actions' => [
        'preview' => 'Anteprima',
        'open_preview' => 'Apri anteprima',
        'add_slide' => 'Aggiungi slide',
        'add_batch_to_carousel' => 'Aggiungi alla galleria',
        'save_settings' => 'Salva impostazioni',
        'restore_aurora_background' => 'Ripristina Aurora',
        'save_tagline' => 'Salva slogan',
        'apply_smtp_preset' => 'Applica preset',
        'add_contact_desk' => 'Aggiungi sportello',
        'clear_cache' => 'Svuota cache',
        'back_to_site' => 'Torna al sito',
    ],

    'locale' => [
        'label' => 'Lingua CMS',
        'switch_to' => 'Passa a :locale',
        'names' => [
            'it' => 'Italiano',
            'en' => 'Inglese',
        ],
    ],

    'preview' => [
        'description' => 'Salva le modifiche correnti e apre l\'anteprima in una nuova scheda.',
        'article_description' => 'Salva le modifiche correnti e apre l\'anteprima dell\'articolo in una nuova scheda.',
        'unavailable' => 'Anteprima non disponibile',
        'add_slug' => 'Aggiungi prima uno slug per la lingua selezionata.',
    ],

    'notifications' => [
        'tagline_saved' => 'Slogan salvato',
        'integrations_saved' => 'Impostazioni integrazioni salvate',
        'sportelli_config_saved' => 'Configurazione sportelli salvata',
        'captcha_saved' => 'Impostazioni captcha salvate',
        'social_links_saved' => 'Link social salvati',
        'site_appearance_saved' => 'Aspetto salvato',
        'site_appearance_restored' => 'Sfondo Aurora ripristinato',
        'donations_config_saved' => 'Configurazione donazioni salvata',
        'recurring_donation_saved' => 'Donazione ricorrente salvata',
        'smtp_preset_applied' => 'Preset SMTP applicato: :provider',
        'smtp_preset_missing' => 'Seleziona un provider prima di applicare il preset',
        'contact_desks_invalid' => 'Elenco sportelli non valido',
        'espocrm_sync_failed' => 'Sincronizzazione Finanziamento EspoCRM non riuscita',
        'developer_tools_saved' => 'Impostazioni sviluppatore salvate',
        'cache_cleared' => 'Cache svuotata',
        'cms_locale_updated' => 'Lingua CMS aggiornata',
        'batch_empty' => 'Nessuna foto da aggiungere',
        'batch_merged' => ':count foto aggiunte alla galleria',
        'batch_failed' => 'Caricamento foto non riuscito',
    ],

    'items' => [
        'new_slide' => 'Nuova slide',
        'new_stat' => 'Nuova statistica',
        'new_service_card' => 'Nuova scheda servizio',
    ],

    'validation' => [
        'image_max' => 'L\'immagine supera la dimensione massima consentita.',
        'image_type' => 'Carica un file immagine valido (JPEG, PNG o WebP).',
    ],

    'roles' => [
        'super_admin' => 'Super admin',
        'admin' => 'Admin',
        'journalist' => 'Giornalista',
    ],

];
